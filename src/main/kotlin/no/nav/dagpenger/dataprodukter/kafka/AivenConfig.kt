...
class AivenConfig(
    ...
    companion object {
        val default
            get() =
                AivenConfig(
                    brokers =
                        requireNotNull(System.getenv("KAFKA_BROKERS")) { "Expected KAFKA_BROKERS" }
                            .split(',')
                            .map(String::trim),
    ...
    fun producerConfig(properties: Properties) =
        Properties().apply {
    ...
    fun avroProducerConfig(properties: Properties = Properties()) =
        Properties().apply {
