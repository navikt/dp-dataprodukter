...
internal class GammelSøknadData(
    ...
    private fun eosArbeidsforhold() =
        this
            .getFakta("eosarbeidsforhold")
            .map { it["properties"]["land"].asText() }

    private fun getFakta(faktaNavn: String) = data.get("fakta")?.filter { it["key"].asText() == faktaNavn } ?: emptyList()

    private fun getFaktumValue(fakta: List<JsonNode>) = fakta.first().get("value")

    private fun asÅrsak(type: String) =
        when (type) {
